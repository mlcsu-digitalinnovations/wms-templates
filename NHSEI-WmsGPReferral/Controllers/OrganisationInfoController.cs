...
namespace NHSEI_WmsGPReferral.Controllers
{
    public class OrganisationInfoController : Controller
    {
        ...
        private readonly Dictionary<string, string> validRoleIds = new Dictionary<string, string>
        {
            { "RO177", "Practice" },
            { "RO76", "Practice" },
            { "RO182", "Pharmacy" },
        ...
        public async Task<IActionResult> IndexAsync(OdsModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    PracticeDataModel practiceData = new PracticeDataModel();
                    switch (responseContent)
                    {
                        ...
                        default:
                            ...
                            {
                                OdsCode = model.OdsCode.ToUpper().Trim(),
                                OrganisationName = responseJson.SelectToken("$.Organisation.Name").Value<string>(),
                                OrganisationCountry = responseJson.SelectToken("$.Organisation.GeoLoc.Location.Country").Value<string>().ToLower(),
                                OrganisationRoles = responseJson.SelectTokens("$.Organisation.Roles.Role[*].id").Values<string>()
                            };
                            break;
                    }
                    ...
                    else
                    {
                        ...
                        if (practiceData.OrganisationName != null)
                        {
                            ...
                            bool isGPorPharmacist = practiceData.OrganisationRoles.All(value => validRoleIds.ContainsKey(value));
                            ...
                            if (isGPorPharmacist && isGPorPharmacistInEngland)
                            {
                                ...
                                HttpContext.Session.SetString(SESSION_KEY_ORG_NAME, practiceData.OrganisationName.ToString());
                                HttpContext.Session.SetString(SESSION_KEY_ORGANISATION_ROLE, practiceData.OrganisationRoles.FirstOrDefault().ToString());
                            ...
                            else
                            {
                                ...
                                {
                                    errorMessage = "This service is currently open to GP Practices & Pharmacies in England only";
                                }
                        ...
                        else
                        {
                            ...
                            errorMessage = "There was a problem with the data returned from the organisation lookup. Please try again. If the problem persists, contact us using the details below.";
        ...
        public IActionResult ConfirmOds(string odsCode)
        {
            ...
            {
                PracticeDataModel model = new PracticeDataModel()
                {
            ...
            if (responseContent != "NotFound" && responseContent != "NoContent")
            {
                if (OdsOrgType() == "Pharmacy")
                {
                    ...
                    if (templateVersion != _PharmacyTemplateVersion)                        
                    {
                        ...
                        PostPharmacyWmpDataModel postWmpDataModel = new PostPharmacyWmpDataModel
                        {
                            Email = returnedEmail,
                        ...
                        var putResponse = await _wmpService.UpdatePharmacyItemAsync(postWmpDataModel);                        
        ...
        {
            SelectClinicalSystemModel model = new SelectClinicalSystemModel();
            if (!string.IsNullOrWhiteSpace(HttpContext.Session.GetString(SESSION_KEY_CLINICAL_SYSTEM)))
            {
                SelectListItem selectedSystem = model.ClinicalSystemList.Where(x => x.Value == HttpContext.Session.GetString(SESSION_KEY_CLINICAL_SYSTEM)).First();
        ...
        {
            ConfirmDataModel model = new ConfirmDataModel
            {
                OdsCode = HttpContext.Session.GetString(SESSION_KEY_ODS_CODE),
        ...
        public async Task<IActionResult> ConfirmDataAsync(ConfirmDataModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    PostPracticeWmpDataModel postWmpDataModel = new PostPracticeWmpDataModel
                    {
                        Email = model.Email,
        ...
        public async Task<IActionResult> Download(string id)
        {
            ...
            var blob = await _blobService.Download(id);
            if (blob != null)
            {
